...
def post_drink(token):
    ...
    drink  =  Drink.query.filter_by(id==Drink.id).first() #fetch the last record in the table that I added it with POST   
    return jsonify({
        "success": True, 
        "drinks": [drink.long()]
    }),200
...
def update_drink(token,id):
    try:
        ...
        requested_title = body.get('title')
        requested_recipe = body.get('recipe')
        to_be_updated_row=Drink.query.filter_by(Drink.id==id).one_or_none() #See if we have that id is in our Table ?
