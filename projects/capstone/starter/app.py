...
def create_app(test_config=None):
      ...
      def post_actor(token):
        try:
          ...
          if  requested_name  is None:
                abort(422)
          new_actor = Actor(name=requested_name,age=requested_age,gender=requested_gender) #add the new data to the table as a new record
          new_actor.insert()
          if body is None:
                abort(422)
          return jsonify({
      ...
      def  post_movie(token):
            try:
              ...
              new_movie = Movie(title=requested_title,release_date=requested_release_date,actor_id=requested_actor_id)  #add the new data to the table as a new record
              new_movie.insert()
              if body is None:
                    abort(422)
              return jsonify({
